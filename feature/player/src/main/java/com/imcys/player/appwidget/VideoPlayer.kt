...
import com.imcys.player.appwidget.pip.enterPIPMode
import com.imcys.player.appwidget.pip.isActivityStatePipMode
import com.imcys.player.appwidget.uri.VideoPlayerMediaItem
import com.imcys.player.appwidget.uri.toUri
import com.sockmagic.player.appwidget.utils.setFullScreen
...
fun VideoPlayer(
    ...
    seekBeforeMilliSeconds: Long = 10_000L,
    seekAfterMilliSeconds: Long = 10_000L,
    ...
    LaunchedEffect(usePlayerController) { defaultPlayerView.useController = usePlayerController }

    LaunchedEffect(player) { defaultPlayerView.player = player }
...
internal fun VideoPlayerSurface(
    ...
    LifecycleEventEffect { _, event ->
        when (event) {
            ...
            Lifecycle.Event.ON_DESTROY -> if (autoDispose) {
                player.release()
            }
