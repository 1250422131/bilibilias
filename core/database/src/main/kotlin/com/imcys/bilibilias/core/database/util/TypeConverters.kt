...
object TypeConverters {
    ...
    fun fileTypeToString(value: FileType): String = when (value) {
        FileType.VIDEO -> "VIDEO"
        FileType.AUDIO -> "AUDIO"
        else -> throw IllegalArgumentException("Can't convert enum to string, unknown enum value: $value")
    }

    fun stateToString(value: State): String = when (value) {
        State.START -> "START"
    ...
    fun stringToFileType(value: String): FileType = when (value) {
        "VIDEO" -> FileType.VIDEO
        "AUDIO" -> FileType.AUDIO
        else -> throw IllegalArgumentException("Can't convert value to enum, unknown value: $value")
    }

    fun stringToState(value: String): State = when (value) {
        "START" -> State.START
...
internal fun Task.maptToTaskEntity(): TaskEntity = TaskEntity(
    uri = uri.toString(),
...
internal fun TaskEntity.mapToTask(): Task = Task(
    uri = Uri.parse(uri),
