...
#include "model/task_runtime_info.hpp"
...
using namespace model::task_runtime_info;
...
void handle_media_task(JNIEnv *env, jobject thiz, const TaskRuntimeInfo &info,
                        FFmpegMergeListener &callback) {
    // 获取视频和音频路径
    if (!info.videoMedia.has_value()) {
        callback.on_error("任务中未包含视频媒体资源");
        return;
    }
    // 判断音频列表是否为空
    if (info.audioMediaList.empty()) {
        callback.on_error("任务中未包含音频媒体资源");
        return;
    }

    // 获取输出路径
    if (info.outMediaPath.empty()) {
        callback.on_error("任务中未包含输出媒体路径");
        return;
    }

    const std::string &videoPathStr = info.videoMedia->path;
    const std::string &audioPathStr = info.audioMediaList[0].path;
    const std::string &outputPathStr = info.outMediaPath;
    const auto &subtitles = info.subtitles;
    const auto &coverPath = info.coverPath;

    // 是否嵌入字幕
    bool embedSubtitles = !subtitles.empty();
    // 是否嵌入封面
    bool embedCover = !coverPath.empty();

    const char* videoPath = videoPathStr.c_str();
    const char* audioPath = audioPathStr.c_str();
...
//    av_dict_set(&ofmt_ctx->metadata, "copyright", copyright, 0);
    ...
    if (video_index_in == -1 || audio_index_in == -1) {
        callback.on_error("未找到有效的视频或音频流");
        return;
    }
...
extern "C" JNIEXPORT void JNICALL
Java_com_imcys_bilibilias_ffmpeg_FFmpegManger_handleAppDownloadTask(JNIEnv *env, jobject thiz, jstring taskRuntimeInfo_, jobject listener) {
    auto callback = FFmpegMergeListener::create(env, listener);
    if (taskRuntimeInfo_ == nullptr) {
        callback.on_error("任务运行时信息为空");
        return;
    }
    const char *cstr = env->GetStringUTFChars(taskRuntimeInfo_, 0);
    std::string mStr = cstr;
    env->ReleaseStringUTFChars(taskRuntimeInfo_, cstr);

    try {
        TaskRuntimeInfo taskInfo = model::task_runtime_info::parse(mStr);
        handle_media_task(env, thiz, taskInfo, callback);
    } catch (const std::exception &e) {
        callback.on_error(std::string("解析任务运行时信息失败: ") + e.what());
        return;
    }
    env->ReleaseStringUTFChars(taskRuntimeInfo_, cstr);
}
