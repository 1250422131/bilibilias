...
import com.imcys.bilibilias.core.rules.GrantPostNotificationsPermissionRule
...
//            onNodeWithText(tool).performClick()
//            onNodeWithContentDescription(navigateUp).assertDoesNotExist()
...
//            onNodeWithText(tool).performClick()
//            onAllNodesWithText(tool).assertCountEquals(2)

            // As above but for the interests tab.
//            onNodeWithText(download).performClick()
//            onAllNodesWithText(download).assertCountEquals(2)
        }
    }
...
//            onNode(hasText(tool) and hasTestTag("NiaNavItem")).assertIsSelected()
        }
    }
